    ...
    for h in range(n) :
	    ...
	    for j in range(m):
	       ...
	       # At[i,j,:,:]=A[start_r:end_r,start_c:end_c]
                comm.Send(numpy.ascontiguousarray(A[start_r:end_r,start_c:end_c]),dest=h+1,tag=0) # A'(M/p,N/m)
...
if rank == 0:
    ...
    #plt.xlim(0,1000)
    plt.title('Uncoded Distributed Matrix multiplication with 4 workers ')
    plt.savefig('work_block1.pdf')
    #plt.show()
