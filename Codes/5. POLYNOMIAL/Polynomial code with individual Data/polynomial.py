...
r = 3000 # rows of A
s = 3000 # columns of A and rowsin B
t = 3000
m=3
n=3
straggling_mean = 0.03 # mean of the exponential distribution
...
if rank == 0:
    ...
    x= numpy.linspace(1.0, 2.0, num=N)
...
# Simulate straggling #
	...
	print('yshaped=\n',yshaped)
        ...
        # # Check results: uncomment below lines to check if the result is correct
            total_err+=numpy.linalg.norm(C-numpy.transpose(A).dot(B)) # Frobenius norm of the difference
    
    if rank > n:
        pass

if rank == 0:
    print 'time=', total_time/(niter-1)
    print 'error=', total_err/(niter-1)
