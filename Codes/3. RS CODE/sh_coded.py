...
for iter in range(0,niter):
    ...
    if rank != 0:
        # perform computation #
        print('Aenci=',rank,Aenci)
	print('x=',X)
	#yenci=numpy.zeros([Mp,1])
        yenci = Aenci.dot(X)
	#yenci=numpy.einsum('ij,jk->ik',Aenci,x)
	'''
	for i in range(len(Aenci)):
  	 # iterate through columns of Y
	    for j in range(len(x[0])):
 	      # iterate through rows of Y
 	        for k in range(len(x)):
           	    yenci[i][j] += Aenci[i][k] * x[k][j]  
		    print('yenci[i][j] = ',yenci[i][j]) 
	'''     
	print('yenci=',rank,yenci)
        # Simulate straggling #
	...
	print('A.dot(x)=',o)
        ...
        print(numpy.linalg.norm(y-o)/numpy.sqrt(M)) # Frobenius norm of the difference
