# Distributed matrix-vector multiplication using Cauchy
# Matrix as Generator Matrix
...
size = comm.Get_size()
rank = comm.Get_rank()
SZ = [300, 600, 900, 1200, 2400]
SW = [6, 12, 14]
X1 = 5
zi = 0

workbook = xlsxwriter.Workbook('cauchy_data.xlsx')
        ...
        # Receive from other nodes as well #
                ...
                while count_responses < n:
                    comm.Recv(temp, source=MPI.ANY_SOURCE, tag=iter
                              + 1, status=info)
                    count_responses += 1
                err = numpy.linalg.norm(y - A.dot(x))
